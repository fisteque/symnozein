...
def main() -> int:
    if sys.argv[1:]:
        print("sync_symnozein_without_bridge task does not accept arguments", file=sys.stderr)
        return 2
    ...
    module = load_module("sync_symnozein_without_bridge.py", "noema_sync_symnozein_without_bridge")
    old_argv = sys.argv[:]
    try:
        sys.argv = [
    ...
    print(
        json.dumps(
            {
                "task": "sync_symnozein_without_bridge",
